...
class Utilisies {
  ...
  static Future<List<Category>> getCategories() async {
    ...
    http.Response response =
        await http.get(Uri.parse(Utilisies.host + "api/categorie"));
    if (response.statusCode == 200) {
      List data = jsonDecode(response.body);
      categories = data.map((e) => Category.formJson(e)).toList();
    }

    return categories;
  }

  static Future<List<Plat>> getPlats() async {
    List<Plat> plats = [];

    http.Response response =
        await http.get(Uri.parse(Utilisies.host + "api/plat"));
    if (response.statusCode == 200) {
      List data = jsonDecode(response.body);
      plats = data.map((e) => Plat.fromJson(e)).toList();
    }
